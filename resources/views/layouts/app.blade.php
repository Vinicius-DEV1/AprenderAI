...
<head>
    ...
    |----------------------------------------------------------------------
    |
    | Este script roda SINCRONAMENTE no <head>, antes do body ser renderizado,
        ...
        <aside :class="{
            ...
            <nav class="flex-1 py-6 space-y-1 overflow-y-auto overflow-x-hidden transition-all duration-300"
                ...
                <p x-show="!sidebarCollapsed"
                    class="px-2 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mb-2">Menu Principal</p>
                ...
                <p x-show="!sidebarCollapsed"
                    class="px-2 text-xs font-semibold text-slate-400 dark:text-slate-500 uppercase tracking-wider mt-6 mb-2">Conta</p>
                ...
                @if(in_array(auth()->user()->email, config('admin.emails', [])))
                    <a href="{{ route('admin.dashboard') }}"
                        ...
                        <svg class="w-5 h-5 flex-shrink-0 {{ request()->routeIs('admin.*') ? 'text-red-600 dark:text-red-400' : 'text-slate-400 dark:text-slate-500' }}"
                            fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <div class="px-2 py-4 border-t border-slate-200 dark:border-slate-700 transition-all duration-300">
                <div class="flex items-center gap-2 w-full p-2 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 overflow-hidden"
                    ...
                    <form method="POST" action="{{ route('logout') }}" class="flex-shrink-0" x-show="!sidebarCollapsed">
                        @csrf
                        <button type="submit" class="text-slate-400 dark:text-slate-500 hover:text-red-600 dark:hover:text-red-400 transition-colors p-1"
                            title="Sair">
        ...
        <div class="flex-1 flex flex-col min-w-0 transition-all duration-300" :class="{ 'lg:ml-0': true }">
            ...
            <header
                ...
                <div class="flex items-center gap-3">
                    <button @click="sidebarOpen = true" class="text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-200">
            ...
            <main class="flex-1 p-4 lg:p-8 overflow-y-auto">
                <div class="max-w-7xl mx-auto">
                    ...
                    {{-- Success Alert --}}
                    @if (session('success'))
                        <div class="mb-6 bg-green-50 dark:bg-green-950/30 border-l-4 border-green-500 p-4 rounded-r shadow-sm">
                            <div class="flex">
                    ...
                    {{-- Error Alert --}}
                    @if (session('error'))
                        <div class="mb-6 bg-red-50 dark:bg-red-950/30 border-l-4 border-red-500 p-4 rounded-r shadow-sm">
                            <div class="flex">
                                <div class="flex-shrink-0">
