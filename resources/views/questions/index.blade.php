...
@section('content')
    <style>
        ...
        .qb-overview-card .val {
            font-size: 28px;
            font-weight: 700;
            color: #6366f1;
        }

        .qb-overview-card .lbl {
            font-size: 11px;
            color: #64748b;
            margin-top: 2px;
        }
        ...
        .qb-alt.correct-reveal {
            border-color: #10b981;
            background: #f0fdf4;
        }

        .qb-alt.incorrect-reveal {
            border-color: #ef4444;
            background: #fef2f2;
        }
        ...
        .qb-alt.selected .qb-alt-letter {
            background: #6366f1;
            color: white;
        }

        .qb-alt.correct-reveal .qb-alt-letter {
            background: #10b981;
            color: white;
        }

        .qb-alt.incorrect-reveal .qb-alt-letter {
            background: #ef4444;
            color: white;
        }
        ...
        .qb-feedback.correct .qb-feedback-title {
            color: #065f46;
        }

        .qb-feedback.incorrect .qb-feedback-title {
            color: #991b1b;
        }
    ...
    <div x-data="statsSlideOver()" x-cloak>
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-200" x-transition:enter-start="opacity-0"
            ...
            x-transition:leave-start="opacity-100" x-transition:leave-end="opacity-0" class="qb-slideover-backdrop"
        ...
        <div x-show="open" x-transition:enter="transition ease-out duration-300" x-transition:enter-start="translate-x-full"
            ...
            x-transition:leave-start="translate-x-0" x-transition:leave-end="translate-x-full" class="qb-slideover"
            style="display:none">
    ...
    <div class="qb-header">
        ...
        <div style="display:flex; flex-direction:column; align-items:flex-end; gap:12px">
            ...
            <div class="qb-header-stats">
                ...
                <div class="qb-stat">
                    <div class="val">{{ $overview['accuracy'] }}%</div>
                    <div class="lbl">Acerto</div>
                </div>
                <div class="qb-stat">
                    <div class="val">{{ $overview['correct'] }}</div>
                    <div class="lbl">✓ Acertos</div>
                </div>
                <div class="qb-stat">
                    <div class="val">{{ $overview['incorrect'] }}</div>
                    <div class="lbl">✗ Erros</div>
                </div>
            </div>
        </div>
    ...
    {{-- ===== FILTERS ===== --}}
    <div class="qb-filters" x-data="filterPanel(
            ...
            <div class="qb-filter-row">
                ...
                <div style="display:flex; align-items:flex-end; padding-bottom:2px">
                    <button type="button" class="qb-filter-toggle" @click="moreFilters = !moreFilters">
                        <svg class="w-4 h-4 transition-transform" :class="moreFilters ? 'rotate-180' : ''" fill="none"
            ...
            <div x-show="moreFilters" x-transition class="qb-filter-row">
                ...
                <div class="qb-filter-item" style="max-width:160px">
                    ...
                    <select name="status" x-model="filters.status">
                        <option value="">Todos</option>
                        <option value="unanswered" {{ request('status') == 'unanswered' ? 'selected' : '' }}>Não respondidas
                        </option>
    ...
    @forelse($questions as $question)
        <div class="qb-card"
            x-data="questionCard({{ $question->id }}, {{ json_encode(isset($answeredMap[$question->id])) }}, {{ json_encode($answeredMap[$question->id] ?? null) }})">
            {{-- Meta --}}
            <div class="qb-card-meta">
                ...
                @endphp
                @if($dc)
                    <span class="qb-badge {{ $dc['class'] }}"
                        title="{{ $question->difficulty_reasoning ?? '' }}">{{ $dc['label'] }}</span>
                @endif
                <template x-if="alreadyAnswered">
                    <span class="qb-badge" :class="wasCorrect ? 'qb-badge-correct' : 'qb-badge-incorrect'"
                        x-text="wasCorrect ? '✓ Acertou' : '✗ Errou'"></span>
                </template>
            </div>
            ...
            @foreach($question->alternatives as $letter => $text)
                <div class="qb-alt" :class="{
                    ...
                    <template x-if="answered && '{{ $letter }}' === correctAnswer">
                        <svg class="w-5 h-5 text-green-600 ml-auto flex-shrink-0" fill="none" viewBox="0 0 24 24"
                            stroke="currentColor">
            ...
            <div class="qb-card-actions">
                <button class="qb-action-btn primary" @click="submitAnswer()"
                    :disabled="!selectedAnswer || answered || submitting" x-show="!answered">
            ...
            <div x-show="showChat" x-transition x-cloak class="qb-chat-container">
                <div class="qb-chat-history space-y-2 p-1" x-ref="chatHistory">
                    <template x-for="msg in chatMessages" :key="msg.id || msg.created_at">
                        <div
                            :class="msg.role === 'user' ? 'flex justify-end' : (msg.role === 'system' ? 'flex justify-center' : 'flex justify-start')">
                            <template x-if="msg.role === 'user'">
                                <div class="rounded-lg px-3 py-1.5 max-w-[85%] text-xs shadow-sm"
                            ...
                            <template x-if="msg.role === 'assistant'">
                                <div
                                    class="bg-white border border-gray-200 rounded-lg px-3 py-1.5 max-w-[85%] text-xs shadow-sm">
                            ...
                            <template x-if="msg.role === 'system'">
                                <div class="bg-red-50 border border-red-200 rounded-lg p-3 text-center w-[90%]">
                                    ...
                                    <a :href="msg.upgrade_url || '/'"
                                        class="mt-2 inline-block bg-gradient-to-r from-red-500 to-orange-500 text-white text-xs font-bold py-1.5 px-4 rounded-full">🚀
                                        Turbinar Plano</a>
                                </div>
                            </template>
                        </div>
                    ...
                    <div x-show="chatTyping" class="flex items-start">
                        <div
                            class="bg-gray-100 rounded-lg px-3 py-2 text-xs text-gray-500 flex items-center gap-2 border border-gray-200">
                ...
                <div class="flex gap-2 mt-2">
                    <input type="text" x-model="chatInput" @keydown.enter.prevent="sendChat()" placeholder="Tire sua dúvida..."
                        :disabled="chatTyping"
                        class="flex-1 rounded-md border border-gray-300 shadow-sm text-xs px-3 py-2 focus:border-indigo-500 focus:ring-indigo-500">
    ...
    @empty
        <div class="qb-card" style="text-align:center; padding:48px">
            ...
            <p style="font-size:13px; color:#cbd5e1">Tente ajustar seus filtros ou <a href="{{ route('questions.index') }}"
                    style="color:#6366f1">limpar a busca</a>.</p>
        </div>
    @endforelse
