...
function ApiAnalyticsDash({
    ...
    const dailyChartOptions: ApexCharts.ApexOptions = {
        ...
        tooltip: { theme: 'light', y: { formatter: (val) => `${val} requisi├º├Áes` } }
    };
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h3 className="text-xl font-black text-slate-800 tracking-tight flex items-center gap-2">
                            Consumo de Intelig├¬ncia Artificial
                            {isFetching && <motion.span animate={{ rotate: 360 }} transition={{ repeat: Infinity, duration: 1 }} className="text-indigo-400 text-sm">Ôîø</motion.span>}
                        </h3>
                        <p className="text-xs font-bold text-slate-400 mt-0.5 uppercase tracking-wider">Monitoramento Din├ómico de Custos e Performance</p>
                    </div>
                </div>
                    ...
                    <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                        <div className="p-6 bg-slate-50/50">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-8 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Per├¡odo (In├¡cio)</label>
                                    <input
                                        type="date"
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Per├¡odo (Fim)</label>
                                    <input
                                        type="date"
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">M├│dulo / Recurso</label>
                                    <select
                                ...
                                <div className="space-y-1">
                                    <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest pl-1">Modelo Espec├¡fico</label>
                                    <input
                                        type="text"
                            ...
                            <div className="flex gap-6 mb-8 pl-2">
                                <div>
                                    <p className="text-xs font-black text-slate-400 uppercase tracking-wider">Requisi├º├Áes Filtradas</p>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
                                <div className="lg:col-span-2 bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                                    <h4 className="text-sm font-bold text-slate-800 mb-4 bg-slate-50 inline-block px-3 py-1 rounded-lg">Tend├¬ncia de Uso Di├írio</h4>
                                    <div className="h-[280px]">
                                        <Chart options={dailyChartOptions} series={[{ name: 'Requisi├º├Áes', data: dailyRequests }]} type="area" height="100%" />
                                    </div>
                                </div>
                                <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100">
                                    <h4 className="text-sm font-bold text-slate-800 mb-4 bg-slate-50 inline-block px-3 py-1 rounded-lg">Distribui├º├úo por M├│dulo</h4>
                                    {moduleRequests.length > 0 ? (
                                        <div className="h-[280px]">
                                            <Chart options={moduleChartOptions} series={[{ name: 'Requisi├º├Áes', data: moduleRequests }]} type="bar" height="100%" />
                                        </div>
                                    ) : (
                            ...
                            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                                <div className="p-4 bg-slate-50 border-b border-slate-100 flex justify-between items-center">
                                    <h4 className="text-sm font-bold text-slate-800 tracking-tight">Logs Detalhados de Opera├º├Áes</h4>
                                ...
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-[11px]">
                                        <thead className="bg-slate-50/50 text-slate-400 uppercase font-black border-b border-slate-100">
                                            <tr>
                                                <th className="px-6 py-4">Hor├írio Exacto</th>
                                                <th className="px-6 py-4">Usu├írio / Requisitante</th>
                        ...
                        <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-12">
                            <div className="p-6 border-b border-slate-50">
                                <h3 className="text-xl font-black text-slate-800 tracking-tight">Ranking de Consumo (Usu├írios)</h3>
                                <p className="text-xs font-bold text-slate-400 mt-0.5 uppercase tracking-wider">Top 20 Usu├írios por Consumo de IA</p>
                            </div>
                            <div className="p-6">
                                <div className="overflow-x-auto">
                                    <table className="w-full text-left text-sm">
                                        <thead className="text-xs text-slate-400 uppercase font-black border-b border-slate-100">
                                            <tr>
                                                <th className="px-6 py-3">Usu├írio</th>
                                                <th className="px-6 py-3 text-center">Requisi├º├Áes</th>
                                        ...
                                        <tbody className="divide-y divide-slate-50">
                                            ...
                                            {aiRanking.length === 0 && (
                                                <tr>
                                                    <td colSpan={4} className="px-6 py-12 text-center text-slate-400 italic">Nenhum ranking dispon├¡vel para os filtros selecionados.</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
...
export default function AdminApiKeys() {
    ...
    if (isError) return (
        <div className="p-12 flex flex-col items-center justify-center min-h-[400px] text-red-500 gap-4 bg-red-50 rounded-3xl m-8">
            ...
            <div className="text-center">
                <p className="font-black text-xl mb-2">Falha Cr├¡tica na Comunica├º├úo</p>
    ...
    const capabilityMeta: Record<string, { icon: string, description: string }> = {
        chat_tutor: { icon: '­ƒÆ¼', description: 'Responde d├║vidas dos alunos sobre quest├Áes resolvidas.' },
        questions: { icon: 'Ô£ì´©Å', description: 'Gera quest├Áes in├®ditas, explica├º├Áes e gabaritos comentados.' },
        triage: { icon: 'ÔÜÖ´©Å', description: 'Classifica e modera quest├Áes durante o processamento em lote.' },
        search: { icon: '­ƒöì', description: 'Interpreta buscas em linguagem natural na barra de pesquisa.' },
        essays: { icon: '­ƒôØ', description: 'Corrige e pontua reda├º├Áes enviadas pelos alunos.' },
        study_plans: { icon: '­ƒôà', description: 'Cria cronogramas din├ómicos baseados no desempenho real.' },
        embedding: { icon: '­ƒº¼', description: 'Gera vetores sem├ónticos para indexa├º├úo e busca vetorial.' },
        query_embedding: { icon: '­ƒºá', description: 'Gera embeddings paralelos para busca sem├óntica em tempo real.' },
        general: { icon: '­ƒöä', description: 'Uso de prop├│sito geral quando sem capaiblity espec├¡fica.' }
    };
    ...
    return (
        <div className="p-4 md:p-6 w-full space-y-6 bg-slate-50/30 min-h-screen">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
                <div>
                    <h1 className="text-3xl font-black text-slate-800 tracking-tight">SRE Dashboard: APIs ­ƒñû</h1>
                ...
                {data.has_recent_errors && (
                    <motion.div
                        ...
                        <div className="flex items-center gap-3">
                            <span className="text-red-500 text-xl font-bold">ÔÜá´©Å</span>
                            <p className="text-sm font-bold text-red-700">ALERTA CR├ìTICO: Erros detectados nas ├║ltimas 6h.</p>
                        </div>
                    </motion.div>
                )}
            </div>
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div
                    ...
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-3">
                        <span className="p-2 bg-indigo-50 text-indigo-600 rounded-lg text-sm">­ƒöÉ</span>
                            ...
                            <div className="p-8 grid grid-cols-1 lg:grid-cols-3 gap-10">
                                    ...
                                    <div className="flex justify-between items-center bg-slate-50 p-3 rounded-xl mb-1">
                                        ...
                                        {editingVaultId && (
                                            <button
                                                ...
                                                className="text-[10px] font-black text-indigo-600 uppercase hover:underline">Cancelar Edi├º├úo</button>
                                        )}
                                    </div>
                                    <div className="space-y-3">
                                        ...
                                        <div>
                                            ...
                                            <input
                                                ...
                                                className="w-full px-4 py-2 rounded-lg border border-slate-200 outline-none focus:ring-2 focus:ring-indigo-500 text-sm"
                                                placeholder={editingVaultId ? 'ÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇóÔÇó' : 'sk-...'} />
                                        </div>
                                        <button
                                ...
                                <div className="lg:col-span-2 space-y-4">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            ...
                                            <div key={vk.id} className="p-4 rounded-xl border border-slate-100 bg-white shadow-sm hover:border-indigo-200 transition-all flex justify-between items-center group">
                                                <div className="flex-1 min-w-0">
                                                    ...
                                                    <p className="text-[10px] font-bold text-slate-400 mt-1 uppercase tracking-tighter">
                                                        Status: {vk.is_valid ? <span className="text-emerald-500">Ô£à Validada</span> : <span className="text-amber-500">ÔØô N├úo Testada</span>}
                                                    </p>
                                                </div>
                                                <div className="flex items-center gap-1">
                                                    ...
                                                    <button
                                                        onClick={() => {
                                                            if (window.confirm(`Tem certeza que deseja excluir a chave "${vk.nickname}" do cofre? Esta a├º├úo n├úo pode ser desfeita.`)) {
                                        ...
                                        {vaultKeys.length === 0 && <p className="col-span-2 text-center py-10 text-slate-400 italic">O cofre est├í vazio.</p>}
                                    </div>
                                </div>
                            </div>
                        </motion.div>
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div
                    ...
                    className="p-6 border-b border-slate-50 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-colors">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-3">
                        <span className="p-2 bg-purple-50 text-purple-600 rounded-lg text-sm">­ƒÄ»</span>
                        Configura├º├úo por Funcionalidade (Roteamento)
                    </h3>
                            ...
                            <div className="p-8 space-y-8">
                                        ...
                                        <div className="flex gap-2">
                                            <div className="flex-1 px-4 py-3 bg-slate-100 border border-slate-200 rounded-xl text-sm font-mono text-slate-600 flex items-center">
                                                {routingForm.preferred_model ? (
                                                    <span className="flex items-center gap-2"><span className="text-emerald-500">Ô£¿</span> {routingForm.preferred_model}</span>
                                                ) : <span className="text-slate-400 italic">Descoberta Necess├íria...</span>}
                                            </div>
                                            <button
                                                ...
                                                {discoveryLoading ? <span className="animate-spin inline-block">Ôîø</span> : '­ƒöì'}
                                            </button>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="p-6 bg-slate-50/50 rounded-2xl border border-slate-100">
                                    ...
                                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                                        {Object.entries(availableCapabilities).map(([code, label]) => {
                                            const meta = capabilityMeta[code] || { icon: '­ƒñû', description: 'Configurar funcionalidade da IA' };
                                            return (
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
                <div
                    ...
                    className="p-6 border-b border-slate-50 flex justify-between items-center cursor-pointer hover:bg-slate-50 transition-colors">
                    <h3 className="text-lg font-bold text-slate-800 flex items-center gap-3">
                        <span className="p-2 bg-emerald-50 text-emerald-600 rounded-lg text-sm">ÔÜí</span>
                        ...
                        <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-hidden">
                            <div className="p-8 bg-slate-50/50 space-y-8">
                                <p className="text-sm text-slate-500 font-medium italic">
                                    (Ôä╣) Arraste e solte os provedores para definir a ordem de tentativa. O sistema usar├í o primeiro online.
                                </p>
                                            ...
                                            <div key={cap} className="bg-white border border-slate-200 rounded-2xl overflow-hidden shadow-sm flex flex-col">
                                                <div className="bg-indigo-50/30 px-4 py-3 border-b border-indigo-100 flex justify-between items-center">
                                                    <h4 className="font-bold text-indigo-900 text-sm flex items-center gap-2">
                                                        <span className="text-indigo-400">ÔÜí</span> {label}
                                                    </h4>
                                                                ...
                                                                <Reorder.Item
                                                                    ...
                                                                    <div className="flex items-center gap-2">
                                                                        ...
                                                                        <div className="flex divide-x divide-slate-100 border border-slate-100 rounded-lg overflow-hidden bg-slate-50/50">
                                                                            <button type="button" onClick={() => retestMutation.mutate(key.id)} className="p-1.5 hover:bg-slate-100 text-amber-600">ÔÜí</button>
                                                                            <button type="button" onClick={() => key.pivot?.id && deleteApiKeyMutation.mutate(key.pivot.id)} className="p-1.5 hover:bg-red-50 text-red-500">Ô£ò</button>
                                                                        </div>
                                                                    </div>
                                                                </Reorder.Item>
                                                            ))}
            ...
            <section className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden mb-8">
                <div className="p-6 border-b border-slate-50 flex justify-between items-center">
                    <h3 className="font-bold text-slate-800 flex items-center gap-3">
                        <span className="p-2 bg-slate-100 text-slate-600 rounded-lg text-xs">­ƒôè</span>
                        Hist├│rico de Uso (IA Logs)
                    </h3>
                        ...
                        <motion.div initial={{ height: 0 }} animate={{ height: 'auto' }} exit={{ height: 0 }} className="overflow-x-auto">
                            <table className="w-full text-left text-xs">
                                <thead className="bg-slate-50 text-slate-400 uppercase font-bold">
                                    <tr>
                                        <th className="px-6 py-4">Usu├írio</th>
                                        <th className="px-6 py-4">M├│dulo</th>
                                        <th className="px-6 py-4">Hor├írio</th>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-2 bg-white rounded-2xl shadow-sm border border-slate-100 p-6">
                    <h3 className="font-bold text-slate-800 mb-6 flex items-center gap-3">
                        <span className="p-2 bg-amber-50 text-amber-600 rounded-lg text-xs">­ƒöö</span>
                        ...
                        {events.map((e, idx) => (
                            <div key={idx} className="flex items-start gap-4 p-3 bg-slate-50/50 rounded-xl border border-slate-100">
                                <span className={`mt-0.5 text-lg ${e.type === 'success' ? 'text-emerald-500' : e.type === 'error' ? 'text-red-500' : 'text-amber-500'}`}>
                                    {e.type === 'success' ? 'Ô£à' : e.type === 'error' ? 'ÔØî' : 'ÔÜá´©Å'}
                                </span>
                                <div className="flex-1">
                                    <div className="flex justify-between">
                                        <p className="text-xs font-black uppercase text-slate-400 tracking-widest">{e.provider} ÔÇó {new Date(e.created_at).toLocaleTimeString()}</p>
                ...
                <section className="bg-slate-900 rounded-3xl p-8 text-white shadow-2xl relative overflow-hidden flex flex-col h-full min-h-[500px]">
                    ...
                    <h3 className="text-xl font-black mb-8 flex items-center gap-3">
                        <span className="text-2xl">­ƒÅå</span> Maiores Consumidores
                    </h3>
                    <div className="space-y-8 flex-1 overflow-y-auto pr-2">
                        ...
                        {aiRanking.length === 0 && <p className="text-center text-slate-500 py-10 font-medium italic">Ranking indispon├¡vel.</p>}
                    </div>
                </section>
            </div>
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4">
                    ...
                    <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="relative bg-white rounded-3xl shadow-2xl max-w-lg w-full overflow-hidden border border-slate-100">
                        <div className="bg-indigo-600 p-6 flex justify-between items-center text-white">
                            <h3 className="font-black flex items-center gap-2">­ƒñû Modelos Dispon├¡veis</h3>
                            <button onClick={() => setShowModelModal(false)}>Ô£ò</button>
                        </div>
                        <div className="p-4 max-h-[60vh] overflow-y-auto space-y-2">
                            {(discoveredModels || []).map((m: any) => (
                                <div
                                    ...
                                    <span className="text-indigo-600 font-bold opacity-0 group-hover:opacity-100 transition-all">Selecionar ÔåÆ</span>
                                </div>
                            ))}
                ...
                <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-10">
                    ...
                    <motion.div initial={{ y: 50, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="relative bg-white rounded-3xl shadow-2xl w-full max-w-5xl h-full max-h-[90vh] flex flex-col overflow-hidden border border-slate-100">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <h3 className="text-xl font-black text-slate-800">Detalhes da Transa├º├úo IA</h3>
                            <button onClick={() => setShowLogModal(false)} className="p-2 hover:bg-slate-100 rounded-full">Ô£ò</button>
                        </div>
                        <div className="flex-1 overflow-y-auto p-8 space-y-8 font-sans">
                            <div className="grid grid-cols-2 md:grid-cols-6 gap-6">
                                <div><p className="text-[10px] font-black uppercase text-slate-400 mb-1">Usu├írio</p><p className="font-bold text-slate-800">{activeLog.user?.name || 'Sistema'}</p></div>
                                <div><p className="text-[10px] font-black uppercase text-slate-400 mb-1">Hor├írio Exato</p><p className="font-bold text-slate-800">{new Date(activeLog.created_at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit', second: '2-digit' })}</p></div>
                                ...
                                <div><p className="text-[10px] font-black uppercase text-slate-400 mb-1">Tempo Execu├º├úo</p><p className="font-bold text-slate-800">{(Number(activeLog.execution_time) || 0).toFixed(3)}s</p></div>
                                        ...
                                        {showRawResponse ? (
                                            ...
                                            (() => {
                                                try {
                                                    ...
                                                    // Fallback para texto original se n├úo for JSON
