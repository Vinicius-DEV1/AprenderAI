...
export default function AppLayout() {
    ...
    const getNavLinkClass = (pattern: string) => {
        ...
        const baseClass = 'flex items-center rounded-lg text-sm font-medium transition-all duration-200';
    ...
    return (
        <div className="min-h-screen flex text-slate-800 dark:text-slate-200 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            ...
            >
                ...
                {/* Logo Area */}
                <div className={`flex items-center h-20 border-b border-slate-100 dark:border-slate-700 bg-gradient-to-br from-blue-500 to-purple-600 transition-all duration-300 overflow-hidden ${sidebarCollapsed ? 'justify-center px-0' : 'justify-between px-6'
                    }`}>
                    <NavLink to="/dashboard" className="flex items-center gap-2 text-white font-bold text-xl whitespace-nowrap">
                        ...
                        {!sidebarCollapsed && <span>{config.appName || 'Sistema AI'}</span>}
                    </NavLink>
                ...
                <nav className={`flex-1 py-6 space-y-1 overflow-y-auto overflow-x-hidden transition-all duration-300 ${sidebarCollapsed ? 'px-2' : 'px-3'}`}>
                    ...
                    <NavLink to="/study-plan" className={getNavLinkClass('/study-plan')} title="Plano de Estudos">
                        <svg className={getIconClass('/study-plan')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                    ...
                    {isAdmin && (
                        <NavLink to="/admin" className={getNavLinkClass('/admin')} title="Painel Admin">
                            <svg className={getIconClass('/admin', 'red')} fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            <div className="flex-1 flex flex-col min-w-0 transition-all duration-300 lg:ml-0">
                ...
                <header className="lg:hidden flex items-center justify-between h-16 px-4 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-700 shadow-sm">
                    <div className="flex items-center gap-3">
                        ...
                        <span className="font-bold text-lg text-slate-800 dark:text-slate-100">{config.appName || 'Sistema AI'}</span>
                    </div>
