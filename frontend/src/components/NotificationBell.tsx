...
export default function NotificationBell({ openUpward = false }: Props) {
    ...
    // Render the dropdown using a Portal to escape container clipping (overflow: hidden)
    const renderDropdown = () => {
        ...
        return createPortal(
            <div 
                ref={panelRef}
                style={style}
                className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-700 overflow-hidden animate-in fade-in zoom-in duration-200"
            >
                <div className="flex items-center justify-between px-4 py-3 border-b border-slate-100 dark:border-slate-700 bg-slate-50/50 dark:bg-slate-800/50">
                    ...
                    <div className="flex gap-3 text-xs">
                        ...
                        <a href="/notificacoes" className="text-slate-500 hover:text-blue-600 font-semibold" onClick={() => setIsOpen(false)}>Ver todas</a>
                    </div>
                </div>

                <div className="max-h-[420px] overflow-y-auto divide-y divide-slate-50 dark:divide-slate-800 custom-scrollbar">
                                ...
                                <div className="flex-1 min-w-0">
                                    ...
                                    <div className="flex items-center gap-3 mt-1.5">
                                        ...
                                        {n.action_url ? (
                                            ...
                                            <a href="/notificacoes" className="text-[10px] text-blue-500 hover:underline font-bold" onClick={() => setIsOpen(false)}>
                                                Ver detalhes
                                            </a>
                                        )}
                                    </div>
    ...
    return (
        <div className="inline-block">
            ...
            >
                ...
                {unreadCount > 0 && (
                    <span className="absolute top-1 right-1 flex items-center justify-center w-5 h-5 text-[10px] font-bold text-white bg-red-500 border-2 border-white dark:border-slate-900 rounded-full shadow-sm">
